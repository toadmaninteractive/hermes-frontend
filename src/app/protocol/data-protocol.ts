...
export namespace OrderDirection {
    ...
    export function fromJsonKey(json: Igor.Json.JsonValue): OrderDirection {
        switch (json) {
            ...
            default:
                throw new Error(`Invalid OrderDirection value: ${json}`);
        }
    }
}
...
export class Collection<T> {
    ...
    static instanceJsonSerializer<T>(
        t: Igor.Json.IJsonSerializer<T>
    ): Igor.Json.IJsonSerializer<Collection<T>> {
        return {
...
export class CollectionSlice<T> {
    ...
    static instanceJsonSerializer<T>(
        t: Igor.Json.IJsonSerializer<T>
    ): Igor.Json.IJsonSerializer<CollectionSlice<T>> {
        return {
...
export class TreeNode<T> {
    ...
    static fromJson<T>(json: Igor.Json.JsonValue, t: Igor.Json.IJsonSerializer<T>): TreeNode<T> {
        ...
        obj.item =
            'item' in jsonObject && jsonObject['item'] != null
                ? t.fromJson(jsonObject['item'])
                : null;
        obj.children =
            'children' in jsonObject && jsonObject['children'] != null
                ? Igor.Json.List(TreeNode.instanceJsonSerializer<T>(t)).fromJson(
                      jsonObject['children']
                  )
                : [];
        return obj;
    ...
    static toJson<T>(value: TreeNode<T>, t: Igor.Json.IJsonSerializer<T>): Igor.Json.JsonValue {
        ...
        result['children'] = Igor.Json.List(TreeNode.instanceJsonSerializer<T>(t)).toJson(
            value.children
        );
        return result;
    }
    ...
    static instanceJsonSerializer<T>(
        t: Igor.Json.IJsonSerializer<T>
    ): Igor.Json.IJsonSerializer<TreeNode<T>> {
        return {
...
export class BadRequestError<T> extends Error {
    ...
    static instanceJsonSerializer<T>(
        t: Igor.Json.IJsonSerializer<T>
    ): Igor.Json.IJsonSerializer<BadRequestError<T>> {
        return {
